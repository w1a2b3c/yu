...
<nav class="navbar navbar-fixed-top navbar-default">
    <div class="container">
      <div class="navbar-header">
        <span class="navbar-brand">牛角源码网卡密系统安装向导</span>
      </div>
    </div>
  </nav>
...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">牛角源码网卡密系统</h3>
	</div>
	<div class="panel-body">
		<p><iframe src="https://wxfaka.www.njymz.com" style="width:100%;height:465px;"></iframe></p>
		<?php if($installed){ ?>
...
echo <<<HTML
		...
		（如果已事先填写好../includes/config.php相关数据库配置，请 <a href="?do=3&jump=1">点击此处</a> 跳过这一步！）
HTML;
?>
	</div>
</div>
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from ".$dbconfig['dbqz']."_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过卡密系统</div>
...
?>";
		...
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if(DB::query("select * from site_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过卡密系统</div>
				<div class="list-group-item">
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%">
	...
	<div class="panel-body">
<?php
	@file_put_contents("install.lock",'各类高端源码出售QQ97013266');
	clearpack();
